  ...
  Lexer = (function() {
        ...
        if (token) {
          if (token.type === 'MINDENT') {
            if (this.Tokens[i + 1] && this.Tokens[i - 1]) {
              if (((ref1 = this.Tokens[i + 1].type) === ']' || ref1 === ')') && this.Tokens[i - 1].type === 'OUTDENT') {
          ...
          if (token.type === '->') {
            if (this.Tokens[i + 1] && this.Tokens[i + 1].type === 'MINDENT') {
              this.Tokens.splice(i + 1, 0, {
                type: 'INDENT'
              }, {
                type: 'OUTDENT'
              });
    ...
    Lexer.prototype.getTokenFromCode = function(code) {
      ...
      switch (code) {
        ...
        case 40:
          this.colAdvance();
          if (ref = this.lastToken(), indexOf.call(KEYWORDS.CALLABLE, ref) >= 0) {
    ...
    Lexer.prototype.readTokenEndTag = function() {
      ...
      while (this.Track.indent.openedIndent) {
        ...
        tokens.unshift({
          type: 'OUTDENT',
          length: reversed[this.Track.indent.openedIndent - 1]
        });
    ...
    Lexer.prototype.readLineTerminator = function() {
      var ref;
      this.colAdvance();
      if ((ref = this.input.charCodeAt(this.pos)) === 10 || ref === 13 || ref === 8232) {
        return this.readLineTerminator();
      }
      return this.addToken({
        type: 'LINETERMINAROR'
    ...
    Lexer.prototype.readTokenIdentifier = function() {
      ...
      if (this.lastToken() === '=>' && indexOf.call(KEYWORDS.CASTTYPE, value) >= 0) {
        return this.addToken({
          type: 'CASTTYPE',
          value: value
        });
    ...
    Lexer.prototype.readIndent = function() {
      var indent, indentLevel, j, len, reversed, tokens;
    ...
    Lexer.prototype.reversedIndentStack = function() {
