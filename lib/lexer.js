...
(function() {
  var KEYWORDS, Lexer, Position, REGEX, Token,
  ...
  Lexer = (function() {
    ...
    Lexer.prototype.readTokenIdentifier = function() {
      var indentTracker, length, ref, ref1, ref2, res, token, value;
      token = (new Token).setStart(this.getPos());
      ...
      if (indexOf.call(KEYWORDS.RESERVED, value) >= 0) {
        ...
        if (value === 'else') {
          ...
          return this.addToken(res.concat(token.set('type', 'ELSE')).concat(this.lookLinearBlock(token.location)));
        }
        if (value === 'try') {
      ...
      if (indexOf.call(KEYWORDS.PHPRESERVED, value) >= 0) {
        return this.addToken(token.set('type', 'UNEXPECTED').set('value', value));
      }
    ...
    Lexer.prototype.lookLinearBlock = function(loc) {
      var indentTracker, length, next, tokens;
      tokens = [];
  ...
  KEYWORDS = {
    ...
    LOGIC: ['and', 'or', 'xor'],
    RESERVED: ['abstract', 'as', 'break', 'by', 'catch', 'case', 'class', 'clone', 'const', 'continue', 'cte', 'declare', 'delete', 'echo', 'else', 'extends', 'final', 'finally', 'for', 'func', 'goto', 'if', 'implements', 'in', 'instanceof', 'interface', 'loop', 'namespace', 'new', 'not', 'null', 'of', 'private', 'protected', 'public', 'return', 'static', 'switch', 'then', 'throw', 'try', 'unless', 'until', 'use', 'when', 'while'],
