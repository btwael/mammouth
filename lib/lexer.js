  ...
  Lexer = (function() {
    ...
    Lexer.prototype.readTokenIdentifier = function() {
      ...
      if (indexOf.call(KEYWORDS.RESERVED, value) >= 0) {
        if ((value === 'if') && !((ref1 = this.last().type) === 'INDENT' || ref1 === 'MINDENT' || ref1 === 'OUTDENT' || ref1 === '(' || ref1 === 'CALL_START' || ref1 === ',')) {
          value = 'POST_IF';
        }
        if (value === 'then') {
    ...
    Lexer.prototype.rewrite = function() {
      var i, j, len, ref, results, token;
      ref = this.Tokens;
      results = [];
      for (i = j = 0, len = ref.length; j < len; i = ++j) {
        token = ref[i];
        if (token) {
          if (token.type === 'MINDENT' && this.Tokens[i + 1] && this.Tokens[i + 1].type === 'ELSE') {
