...
(function() {
  var Access, Array, ArrayKey, Assign, Base, Block, Break, Call, Class, ClassLine, Clone, Code, Constant, Context, Continue, Declare, Delete, Document, Echo, Else, ElseIf, Existence, Expression, For, GetKeyAssign, Goto, HereDoc, Identifier, If, Interface, Literal, Namespace, NewExpression, Operation, Param, Parens, QualifiedName, Range, RawText, Return, Script, Section, Slice, Switch, Throw, Try, Unary, Update, Value, While, typeCasting,
  ...
  Block = exports.Block = (function(superClass) {
    ...
    Block.prototype.prepare = function() {
      var expression, i, instruction, j, len, ref;
      ref = this.body;
    ...
    Block.prototype.compile = function(system) {
      var code, i, instruction, j, len, ref;
      code = '';
      if (this.braces) {
        code += '{';
      }
      if (this.body.length === 1 && this.body[0].type === 'Expression' && !this.expands) {
        code += ' ' + this.body[0].prepare(system).compile(system) + ' }';
      } else {
        system.indent.up();
        code += '\n';
        ref = this.body;
  ...
  Range = exports.Range = (function(superClass) {
    ...
    Range.prototype.prepare = function() {
      if (this.from instanceof Value && this.from.value instanceof Literal && typeof this.from.value.value === 'number') {
        this.from = this.from.value.value;
      }
      if (this.to instanceof Value && this.to.value instanceof Literal && typeof this.to.value.value === 'number') {
        this.to = this.to.value.value;
      }
      if (typeof this.from === 'number' && typeof this.to === 'number' && Math.abs(this.from - this.to) <= 20) {
    ...
    Range.prototype.compile = function(system) {
      var array, i, j, k, ref, ref1, ref2, ref3, results, results1;
      if (this.compileResult === 'Array') {
        array = this.exclusive ? (function() {
          results = [];
          for (var j = ref = this.from, ref1 = this.to; ref <= ref1 ? j < ref1 : j > ref1; ref <= ref1 ? j++ : j--){ results.push(j); }
          return results;
        }).apply(this) : (function() {
          results1 = [];
          for (var k = ref2 = this.from, ref3 = this.to; ref2 <= ref3 ? k <= ref3 : k >= ref3; ref2 <= ref3 ? k++ : k--){ results1.push(k); }
          return results1;
        }).apply(this);
  ...
  Operation = exports.Operation = (function(superClass) {
    ...
    Operation.prototype.compile = function(system) {
      ...
      return code += this.right.prepare(system).compile(system);
    };
  ...
  Code = exports.Code = (function(superClass) {
    ...
    Code.prototype.prepare = function() {
      if (this.body !== false) {
        this.body.activateReturn(function(exp) {
          return new Expression(new Return(exp));
        });
      }
      return this;
    };
    ...
    Code.prototype.compile = function(system) {
      var code, i, j, len, parameter, ref;
  ...
  If = exports.If = (function(superClass) {
    ...
    If.prototype.compile = function(system) {
      var code, els, j, k, len, len1, ref, ref1;
      if (this.isStatement) {
  ...
  While = exports.While = (function(superClass) {
    ...
    While.prototype.prepare = function(system) {
      ...
      if (this.returnactived) {
        this.cacheRes = cacheRes = system.context.free('result');
    ...
    While.prototype.compile = function(system) {
      ...
      if (this.isStatement) {
        ...
        code += (new Value(new Call(new Code([], new Block([this]))))).prepare(system).compile(system);
      }
      return code;
    };
