...
start
	= (code_block / embedding)*

/* ===== Embedding text or markup ===== */
embedding
	= body:embedding_TEXT_INNER+ { return {type:'embed', content:body.join('')}}

embedding_TEXT_INNER
	= !script_start !script_end c:. {return c}

/* ===== Expression  ===== */
code_block
	= script_start __ LineTerminator els:(statements) script_end { return {type:'block', elements:els}}
...
AssignmentExpressionOfFunction
	= left:LeftHandSideAssignmentExpressionOfFunction __
	operator:("=" (!"=") { return "="; }) __
	...
	/ LeftHandSideAssignmentExpressionOfFunction

LeftHandSideAssignmentExpressionOfFunction
	= '&' name:Identifier { return { type: "ReferenceVariable", name: name }; }
	/ name:Identifier { return { type: "Variable", name: name }; }
...
ExpressionStatement
	= SAMEDENT m:Expression EOL?
		{ return m; }

FunctionInLineCall
	= SAMEDENT m:CallExpression EOL?
		{ return m; }
...
SwitchStatement
	= SwitchToken __ "("? __ variable:Expression __ ")"? __
	cases:(b:blank* INDENT c:(n:SwitchCase)* DEDENT { return b.concat(c); }) 
	elsed:(b:blank* INDENT c:(n:SwitchElse) DEDENT { return c; })? {
...
SwitchCase
	= SAMEDENT CaseToken __ value:Expression __ ThenToken? __ EOL?
	statement:( b:blank* INDENT c:(statement)* DEDENT { return b.concat(c); })? {
		return {
			type: 'case',
			value: value,
			statement: statement
